...
class FiscalRegulatoryCatalogService:
    ...
    def _load(self) -> dict[str, Any]:
        ...
        # O Git pode materializar arquivos de texto com CRLF no Windows. O
        # catálogo é canônico em LF; normalize apenas a quebra de linha antes
        # do hash para não confundir checkout legítimo com adulteração.
        canonical_raw = raw.replace(b"\r\n", b"\n")
        raw_digest = hashlib.sha256(raw).hexdigest().upper()
        canonical_digest = hashlib.sha256(canonical_raw).hexdigest().upper()
        if not self.expected_sha256 or self.expected_sha256 not in {
            raw_digest,
            canonical_digest,
        }:
            raise ValueError(
        ...
        try:
            payload = json.loads(canonical_raw.decode("utf-8"))
