...
class CustomerRegistrationService:
    ...
    def criar(
        ...
        *,
        ...
        if self.history_callback:
            ...
            details = "Cadastro criado." + (f" Operador: {actor}." if actor else "")
            self.history_callback(cliente_id, "CADASTRO", details)
        return cliente_id
    ...
    def editar(
        ...
        *,
        ...
        if self.history_callback:
            actor = str(usuario or "").strip()
            details = "Dados cadastrais atualizados." + (f" Operador: {actor}." if actor else "")
            self.history_callback(normalized_id, "EDIÇÃO", details)

    def excluir_cadastro_sem_movimento(
        self, cliente_id: int, *, usuario: str | None = None,
    ) -> None:
