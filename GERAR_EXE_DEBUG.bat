...
set NOME=NabiCode_v%VERSAO:.=_%_DEBUG
%PYTHON_CMD% -m unittest discover -s tests -v
if errorlevel 1 goto erro
%PYTHON_CMD% -m PyInstaller --noconfirm --clean --name %NOME% --onedir --console --add-data "VERSAO.txt;." --collect-all customtkinter main.py
if errorlevel 1 goto erro
...
echo EXE DEBUG: dist\%NOME%\%NOME%.exe
pause
exit /b 0
:erro
echo FALHA AO GERAR EXE DEBUG.
pause
exit /b 1
