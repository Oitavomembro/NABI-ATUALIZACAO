...
4. Emissão offline completa: ativação no momento da venda, assinatura, QR Code, impressão do DANFE NFC-e de contingência e controle do prazo de transmissão. O lote atual apenas retransmite documentos que já chegaram à fila marcados como contingência.
...
8. Envio de XML e documento auxiliar por e-mail com fila, consentimento e configuração segura.
9. Consulta automática de CNPJ e IE por provedor oficial/contratado, com tratamento de indisponibilidade e limites de uso.
10. NFS-e, que exige arquitetura separada por padrão nacional/provedor municipal e não deve ser simulada pelo motor de NF-e.
11. Política automatizada de retenção, cópia externa e restauração dos documentos fiscais pelo prazo legal aplicável.
12. Homologação estadual além da Bahia, incluindo endpoints, QR Code, regras e testes por UF.

## Funções solicitadas que ainda não existem

- busca visual de NCM/CEST em catálogo oficial atualizado;
- atualização IBPT;
