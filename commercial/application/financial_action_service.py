...
class FinancialActionService:
    ...
    def create_receivable(self, command: CreateFinancialTitleCommand, *, context: ActionContext, confirmed: bool):
        return self._run("CREATE_RECEIVABLE", context, ActionSensitivity.SENSITIVE, confirmed,
                         lambda: self._gateway.create_title("RECEBER", command, user=context.requested_by), "RECEIVABLE_CREATED")

    def create_payable(self, command: CreateFinancialTitleCommand, *, context: ActionContext, confirmed: bool):
        return self._run("CREATE_PAYABLE", context, ActionSensitivity.SENSITIVE, confirmed,
                         lambda: self._gateway.create_title("PAGAR", command, user=context.requested_by), "PAYABLE_CREATED")

    def settle_receivable(self, command: SettleFinancialTitleCommand, *, context: ActionContext, confirmed: bool):
        return self._run("SETTLE_RECEIVABLE", context, ActionSensitivity.SENSITIVE, confirmed,
                         lambda: self._gateway.settle("RECEBER", command, user=context.requested_by), "RECEIVABLE_SETTLED")

    def settle_payable(self, command: SettleFinancialTitleCommand, *, context: ActionContext, confirmed: bool):
        return self._run("SETTLE_PAYABLE", context, ActionSensitivity.SENSITIVE, confirmed,
                         lambda: self._gateway.settle("PAGAR", command, user=context.requested_by), "PAYABLE_SETTLED")
