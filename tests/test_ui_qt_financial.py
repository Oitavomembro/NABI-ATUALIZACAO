...
from commercial.application.financial_dto import FinancialSummary,FinancialTitlePage,ReceivableSummary,PayableSummary
...
class Query:
 ...
 def receivables_page(self,*,limit,offset=0):return FinancialTitlePage((rec(),) if offset==0 else (),1,limit,offset)
 def payables_page(self,*,limit,offset=0):return FinancialTitlePage((pay(),) if offset==0 else (),1,limit,offset)
...
def test_lista_ids_reais_resumo_e_separacao(app):
 d=FinancialDialog(Query(),Actions(),user="ANA",thread_pool=Pool());assert d.receivable_table.item(0,0).data(Qt.ItemDataRole.UserRole)==7;assert "R$ 100,00" in d.summary.text();d.tabs.setCurrentIndex(1);assert d._selected().title_id==8;d.close()
def test_enter_auto_repeat_nao_baixa_e_enter_simples_uma_vez(app,monkeypatch):
 d=FinancialDialog(Query(),Actions(),user="ANA",thread_pool=Pool());calls=[];monkeypatch.setattr(d,"settle",lambda:calls.append(1));d.show();app.processEvents();QApplication.sendEvent(d.receivable_table,QKeyEvent(QEvent.Type.KeyPress,Qt.Key.Key_Return,Qt.KeyboardModifier.NoModifier,"",True,1));assert calls==[];QApplication.sendEvent(d.receivable_table,QKeyEvent(QEvent.Type.KeyPress,Qt.Key.Key_Return,Qt.KeyboardModifier.NoModifier));assert calls==[1];d.close()
...
def test_resposta_atrasada_erro_worker_e_fechamento_sao_descartados(app):
 d=FinancialDialog(Query(),Actions(),user="ANA",thread_pool=Pool()); rows=d.receivables; old=d._generation-1; payload=(FinancialTitlePage((),0,100,0),FinancialTitlePage((),0,100,0),Query().financial_summary());d._loaded(old,payload);assert d.receivables==rows;d._failed(old,"erro antigo");assert "erro antigo" not in d.load_state.text().lower();generation=d._generation;d.close();assert d._generation>generation
