...
def test_central_fiscal_mostra_escolhas_antes_da_grade():
    ...
    assert 'action_panel.pack(fill="x", padx=12, pady=(0, 10))' in block
    ...
    assert 'document_workspace = ctk.CTkFrame(frame' in block
    assert 'result_frame = ctk.CTkFrame(document_workspace' in block
    assert 'tree = ttk.Treeview(result_frame' in block
    assert 'document_layout.show_results()' in block
    ...
    assert "document_layout.hide()" in block
