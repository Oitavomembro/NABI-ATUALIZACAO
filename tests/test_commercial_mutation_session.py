...
def _customer_service(authorizer):
    ...
    service.bind_mutation_authorizer(authorizer)
...
@pytest.mark.parametrize("operation", ["create_customer", "update_customer", "delete_unused_customer"])
def test_customer_empty_actor_blocks_all_writes(operation):
    service, registration = _customer_service(SwitchingAuthorizer(""))
    command = (CustomerCreateCommand(name="CLIENTE") if operation == "create_customer"
               else CustomerUpdateCommand(customer_id=11, name="CLIENTE")
               if operation == "update_customer" else 11)
    with pytest.raises(PermissionError):
        getattr(service, operation)(command)
    assert registration.calls == []


@pytest.mark.parametrize("operation", ["create_receivable", "create_payable", "settle_receivable", "settle_payable", "cancel_financial_title", "reverse_financial_payment"])
def test_financial_revocation_blocks_every_operation(operation):
    authorizer = SwitchingAuthorizer()
    authorizer.allowed = False
    gateway = FinancialGatewaySpy()
    service = FinancialActionService(gateway, mutation_authorizer=authorizer)
    command = (CreateFinancialTitleCommand(Decimal("10"), date.today())
               if operation.startswith("create") else
               SettleFinancialTitleCommand(7, Decimal("10"), "PIX", date.today())
               if operation.startswith("settle") else 7)
    result = getattr(service, operation)(command, context=ActionContext("old", ActionOrigin.UI), confirmed=True)
    assert not result.executed and not result.committed
    assert gateway.calls == []
