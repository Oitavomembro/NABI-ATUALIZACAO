...
class DraftConfirmationService:
    ...
    def issue(self, draft, *, actor: AssistantActor) -> ConfirmationChallenge:
        ...
        challenge = ConfirmationChallenge(
            ...
            required_capability=(
                ...
                if draft.operation_kind in {"PURCHASE_RECEIPT", "NFE_ENTRY_IMPORT", "CUSTOMER_CREATE", "CUSTOMER_RECEIPT"} or draft.operation_kind.startswith("FINANCIAL_")
    ...
    def consume(self, grant, *, draft, operation: str, actor=None) -> _GrantRecord:
        ...
        required = (
            CapabilityLevel.REINFORCED_CONFIRMATION
            if operation in {"PURCHASE_RECEIPT", "NFE_ENTRY_IMPORT", "CUSTOMER_CREATE", "CUSTOMER_RECEIPT"} or operation.startswith("FINANCIAL_")
            else CapabilityLevel.SIMPLE_CONFIRMATION
        )
